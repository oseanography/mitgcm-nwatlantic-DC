C $Header: /u/gcmpack/MITgcm_contrib/high_res_cube/code-mods/EXF_OPTIONS.h,v 1.4 2007/12/23 06:04:43 dimitri Exp $
C $Name:  $
...
c
c   >>> ALLOW_MEGAFLOOD <<<
c       If defined allows user to read in additional freshwater
c       from file: "outburstfile". Designed for simulating
c       the outburst of mega flood events such as the 8.2 Kyr 
c       Lake Agassiz drainage event. Files must be on native model grid
c       e.g. 3060x510x12, not 360x180x12
c
...
C  No longer using define USE_NO_INTERP_RUNOFF
C   If this is defined then model expects 
C    runoff file on model grid. Undefined 
C    will cause the runoff to be interpolated
C    to the model grid based on lat-lon coords
C    given in data.exf
